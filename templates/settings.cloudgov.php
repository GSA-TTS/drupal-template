...
$settings["file_private_path"] = "/home/vcap/private";

$settings["migrate_node_migrate_type_classic"] = false;

$application_fqdn_regex = "^.+\.(app\.cloud\.gov|apps\.internal)$";
...
foreach ($cf_service_data as $service_list) {
    foreach ($service_list as $service) {
        if (stristr($service["name"], getenv("DATABASE_INSTANCE_NAME"))) {
        ...
        } elseif (stristr($service["name"], getenv("SECRETS_INSTANCE_NAME"))) {
        ...
        } elseif (stristr($service["name"], getenv("STORAGE_INSTANCE_NAME"))) {
